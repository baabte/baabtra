angular.module('baabtra').controller('AddmenuCtrl',['$scope','$modal','$rootScope','addMenu','localStorageService',function ($scope,$modal,$rootScope,addMenu,localStorageService){
var loginInfo=localStorageService.get('loginInfo');
  if(loginInfo===null||loginInfo.length===0){
    ...
    {
		$modal({scope: $scope, template: 'angularModules/company/partials/iconPage.html',placement:'center', show: true});
    };
...
$scope.menu.actions=[];
  $scope.addActtion =function()
  {
    if ($scope.menu.stateName && $scope.menu.actionName!= undefined) {
      $scope.menu.actions.push({"actionName":$scope.menu.actionName,"stateName":$scope.menu.stateName});
