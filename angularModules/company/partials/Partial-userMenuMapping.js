...
{
  $scope.companyId="";
	userMenuMappingSrv.FnGetCompanyDetails($scope,'','initial');
}
else if(angular.equals($scope.roleId,2))
{
	...
	userMenuMappingSrv.FnLoadUsers($scope,$scope.companyState,'initial');
}
      ...
      if(!angular.equals(newValue,undefined)){
        CurNewValue=newValue;
          if(angular.equals($scope.SearchType,"Company")){
            userMenuMappingSrv.FnGetCompanyDetails($scope,CurNewValue,'initial');
          }
          else{
            $scope.menudetails=false;
            $scope.UserState=-1;
            //userMenuMappingSrv.FnLoadUsers($scope,$scope.companyState,'',newValue); //commented by akshath for search disable.
          }
        }
      });
	$scope.next_one = function() {
      ...
      //$scope.nvType='next';
			userMenuMappingSrv.FnGetCompanyDetails($scope,CurNewValue,'next');
		}
		else if(angular.equals($scope.SearchType,"User")){
			$scope.range=$scope.range+8;
      //$scope.nvType='next';
			userMenuMappingSrv.FnLoadUsers($scope,$scope.companyState,'next');
		}
	};
	$scope.prev_one = function() {//To get Previous page
		if(angular.equals($scope.SearchType,"Company")){
			$scope.range=$scope.range-6;
     // $scope.nvType='prev';
			userMenuMappingSrv.FnGetCompanyDetails($scope,CurNewValue,'prev');
		}
		else if(angular.equals($scope.SearchType,"User")){
			$scope.range=$scope.range-8;
      //$scope.nvType='prev';
			userMenuMappingSrv.FnLoadUsers($scope,$scope.companyState,'prev');
		}
	};
   ...
   $scope.menudetails=false;
	...
	$scope.tree1NodesOptions = { 
     ...
     $scope.getUsers=function(cmp_id){
        ...
        $scope.SearchType="User";
        $scope.companyState=cmp_id;
        userMenuMappingSrv.FnLoadUsers($scope,'','initial');  //calling the service function FnLoadUsers() for loading the existing users
      };
     $scope.getUserMenu=function(id,fkRoleId){
        $scope.UserState=id;
        userMenuMappingSrv.FnLoadExMenuItems4AUMMapping($scope,id,fkRoleId.$oid,$scope.companyState);
      };
