!function (a, b) {
  b['true'] = a, angular.module('textAngularSetup', []).value('taOptions', {
    ...
    quote: { tooltip: 'Quote/unqoute selection or paragraph' },
    undo: { tooltip: 'Undo' },
    redo: { tooltip: 'Redo' },
    bold: { tooltip: 'Bold' },
    ...
    function c(b, c) {
      if (!b || '' === b || n.hasOwnProperty(b))
        throw 'textAngular Error: A unique name is required for a Tool Definition';
      if (c.display && ('' === c.display || !a(c.display)) || !c.display && !c.buttontext && !c.iconclass)
        throw 'textAngular Error: Tool Definition for "' + b + '" does not have a valid display/iconclass/buttontext value';
      n[b] = c;
    }
    var d = !1;
    /AppleWebKit\/([\d.]+)/.exec(navigator.userAgent) && (document.addEventListener('click', function () {
      ...
      if (d && null !== a) {
        for (var b = !1, c = a; null !== c && 'html' !== c.tagName.toLowerCase() && !b;)
          b = 'true' === c.contentEditable, c = c.parentNode;
    ...
    var e = function () {
        ...
        if (d > 0)
          b = parseInt(c.substring(d + 5, c.indexOf('.', d)), 10);
        else if (e > 0) {
          var f = c.indexOf('rv:');
    ...
    var l = !1, m = angular.module('textAngular', [
        'ngSanitize',
        'textAngularSetup'
      ]), n = {};
      ...
      function (a, b, c, d, e, f, g, h, i, j) {
        return {
          ...
          link: function (k, l, m, n) {
            var o, p, q, r, s, t, u, v, w, x = m.serial ? m.serial : Math.floor(10000000000000000 * Math.random()), y = m.name ? m.name : 'textAngularEditor' + x, z = function (a, c, d) {
                b(function () {
                  var b = function () {
                    a.off(c, b), d();
                  };
      ...
      function (a, b, c) {
        var d = /^(address|article|aside|audio|blockquote|canvas|dd|div|dl|fieldset|figcaption|figure|footer|form|h1|h2|h3|h4|h5|h6|header|hgroup|hr|noscript|ol|output|p|pre|section|table|tfoot|ul|video)$/gi, e = /^(ul|li|ol)$/gi, f = function (b, c) {
            ...
            for (e = f.length - 1; e >= 0; e--)
              d = angular.element('<' + c + '>' + f[e].innerHTML + '</' + c + '>'), b.after(d);
          ...
          return i = b(i), function (j, k, l) {
            ...
            if (void 0 !== s) {
              ...
              if ('insertorderedlist' === j.toLowerCase() || 'insertunorderedlist' === j.toLowerCase()) {
                ...
                if (u === v)
                  return f(t, i);
                if ('li' === u && t.parent()[0].tagName.toLowerCase() === v && 1 === t.parent().children().length)
                  return f(t.parent(), i);
                if ('li' === u && t.parent()[0].tagName.toLowerCase() !== v && 1 === t.parent().children().length)
                  return g(t.parent(), v);
                ...
                if (u.match(d)) {
                  if (p = a.getOnlySelectedElements(), 1 === p.length && ('ol' === p[0].tagName.toLowerCase() || 'ul' === p[0].tagName.toLowerCase()))
                    return p[0].tagName.toLowerCase() === v ? f(angular.element(p[0]), i) : g(angular.element(p[0]), v);
                  o = '';
                  var x = [];
                ...
                for (n = 'li' === u ? t.parent() : t; !n[0].tagName.match(d);)
                  n = n.parent(), u = n[0].tagName.toLowerCase();
                if (u === z) {
                  p = n.children();
                  var A = !1;
                  for (m = 0; m < p.length; m++)
                    A = A || p[m].tagName.match(d);
                  ...
                  if (u.match(e))
                    ...
                    if (angular.forEach(p, function (a) {
                      ...
                      for (; 3 === p[0].nodeType || !p[0].tagName.match(d);)
                        p = [p[0].parentNode];
                    if (angular.element(p[0]).hasClass('ta-bind'))
                      n = angular.element(l), n[0].innerHTML = p[0].innerHTML, p[0].innerHTML = n[0].outerHTML;
                    else if ('blockquote' === z) {
                      for (o = '', m = 0; m < p.length; m++)
                        o += p[m].outerHTML;
                      ...
                      for (m = 0; m < p.length; m++)
                        n = angular.element(l), n[0].innerHTML = p[m].innerHTML, p[m].parentNode.insertBefore(n[0], p[m]), p[m].parentNode.removeChild(p[m]);
                  }
                else {
                  var C = n.parent(), D = C.contents();
                  for (m = 0; m < D.length; m++)
                    C.parent().hasClass('ta-bind') && 3 === D[m].nodeType && (r = angular.element('<' + i + '>'), r[0].innerHTML = D[m].outerHTML, D[m] = r[0]), C.parent()[0].insertBefore(D[m], C[0]);
                  C.remove();
                }
            ...
            if (j.$parent['updateTaBind' + (q.id || '')] = function () {
              ...
              if (u) {
                if (p.on('cut', function (a) {
                    ...
                    var d;
                    if (a.clipboardData || a.originalEvent && a.originalEvent.clipboardData ? d = (a.originalEvent || a).clipboardData.getData('text/plain') : c.clipboardData && (d = c.clipboardData.getData('Text')), !d && !w)
                      return !0;
                    if (a.preventDefault(), !w) {
                      ...
                      if (e[0].innerHTML = d, d = e.text(), f[0].selection) {
                        ...
                        f[0].execCommand('insertText', !1, d);
                      A();
                    }
      ...
      function (a, b, c) {
        ...
        return {
          ...
          updateToolbarToolDisplay: function (a, b, c) {
            if (!d[a])
              throw 'textAngular Error: No Toolbar with name "' + a + '" exists';
      ...
      function (a, b) {
        var c = b[0], d = function (a) {
            ...
            if (b === c)
              return [b];
            for (var e = []; b && b !== c;)
              b = d(b), b.parentNode === a.commonAncestorContainer && e.push(b);
            for (b = a.startContainer; b && b !== a.commonAncestorContainer;)
              b.parentNode === a.commonAncestorContainer && e.unshift(b), b = b.parentNode;
            return e;
          };
        return {
